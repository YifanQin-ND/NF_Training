...
def train_loop(
    ...
    for epoch in range(epochs):
        ...
        if args.type == 'irs':
            ...
            acc = test_fn_irs(model, device, test_loader, 4, True)
            if acc[-1] > best_acc:
                best_acc = acc[-1]
                log_file.write(f'acc1 is {acc[0]:.2f}%, acc2 is {acc[1]:.2f}%, acc3 is {acc[2]:.2f}%, accb is {acc[3]:.2f}%, loss is {train_loss}\n')
...
def train_part(
    ...
    save_path = f'check_points/vgg8_{args.type}_{args.dataset}_{args.num}_{args.mark}.pth'
    ...
    if args.type == 'base':
        net = vgg8(in_channel, num_classes, noise_backbone=args.var1).to(Config.DEVICE)
        log_file.write('Vgg-8 baseline\n')
    elif args.type == 'irs':
        net = vgg8_irs(in_channel, num_classes, noise_backbone=args.var1, noise_block=args.var2).to(Config.DEVICE)
        log_file.write('Vgg-8 irs\n')
    elif args.type == 'ovf':
        net = vgg8(in_channel, num_classes, noise_backbone=args.var1).to(Config.DEVICE)
        log_file.write('Vgg-8 ovf\n')

    log_file.write(f'Vgg-8 with {args.type}\n')
...
def test_part(args, test_loader):
    ...
    for noise_var in inf_variation:
        if noise_var == 0:
            net = vgg8_test(in_channel, num_classes, noise_backbone=noise_var).to(Config.DEVICE)
