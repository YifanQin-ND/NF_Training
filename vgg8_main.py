...
from config import Config, s_factor, vgg8_beta
...
def train_loop(
    ...
    for epoch in range(epochs):
        ...
        if args.type == 'irs':
            train_loss = train_fn_irs(args, model, device, train_loader, optimizer, criterion, vgg8_beta[args.type][args.dataset])
            acc = test_fn_irs(model, device, test_loader, 4, True)
            print(f'acc1 is {acc[0]:.2f}%, acc2 is {acc[1]:.2f}%, acc3 is {acc[2]:.2f}%, accb is {acc[3]:.2f}%, loss is {train_loss}')
            if acc[-1] > best_acc:
                best_acc = acc[-1]
                best_ep = epoch + 1
...
def train_part(
    ...
    save_path = f'check_points/vgg8_{args.type}_{args.dataset}_{args.mark}_{args.num}.pth'

    print('******Train BEGIN!!!*****')
    print(f'Vgg-8 with {args.type}')
    ...
    print(f'dataset={args.dataset}, beta={vgg8_beta[args.type][args.dataset]}.')
...
def test_part(args, test_loader):
    ...
    for noise_var in inf_variation:
        if noise_var == 0:
            net = vgg8_test(in_channel, num_classes, noise_var, s_factor[args.device]).to(Config.DEVICE)
